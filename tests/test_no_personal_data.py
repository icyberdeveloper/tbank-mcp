...
SHAPES = {
    ...
    # Госномер: буква-3 цифры-2 буквы-регион. Обе раскладки, потому что обе ходят
    # по проводу: MyT принимает бронь с кириллическим номером и возвращает его же
    # транслитом (ГОСТ-набор ABEKMHOPCTYX — те буквы, что совпадают начертанием).
    # Добавлено вместе с парковкой: до неё ни одна форма не ловила номер машины,
    # то есть настоящий номер уехал бы в публичный репозиторий беспрепятственно.
    "госномер": re.compile(
        r"\b(?:[АВЕКМНОРСТУХ]\d{3}[АВЕКМНОРСТУХ]{2}\d{2,3}"
        r"|[ABEKMHOPCTYX]\d{3}[ABEKMHOPCTYX]{2}\d{2,3})\b"),
    # Почта. Тоже из-за MyT: в корпоративном календаре у каждой встречи список
    # участников с рабочими адресами КОЛЛЕГ — людей, которые этот репозиторий не
    # открывали и согласия не давали. Синтетика тут живёт на example.com.
...
ALLOWED = {
    ...
    "ST00012",
    # ИНН-формы — хвосты синтетических UUID из myt-фикстуры и тестов MyT.
    # Счётчик regen_myt.py выдаёт 00000000-0000-4000-8000-0000000000NN, и её
    # последние 12 цифр совпадают по форме с ИНН. Значения счётчиковые, в трафике
    # не встречаются ни разу.
    "000000000005", "000000000006", "000000000007", "000000000008",
    "000000000009", "000000000010", "000000000011", "000000000013",
    "000000000099",
    # госномера — синтетические: regen_myt.py FAKE_CAR_*, примеры в скиле и доках
    "A000AA000", "А000АА000", "X000XX99",
    # почта — только example.com: шаблон скраббера и его вывод в фикстуре
    "user%d@example.com", "user@example.com", "user3@example.com",
    "user7@example.com",
}
