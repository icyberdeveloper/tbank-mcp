...
def myt_status() -> str:
    ...
    Как и любой читающий тул здесь (и как банковский session_status), он может по
    дороге переминтить токен и переписать файл сессии: это поддержание доступа, а не
