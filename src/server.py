...
TOOL_KINDS: dict[str, tuple[str, str]] = {
    ...
    "myt_refresh_session": ("Обновление корпоративной сессии MyT", WRITE),
...
def myt_status() -> str:
    ...
    Отдельного тула «обновить сессию MyT» нет, и он не нужен: токен продлевается
    сам перед каждым запросом, за 120 секунд до истечения. Поэтому нулевой остаток
    НЕ значит «сессия мертва» — следующий же вызов её переминтит. Этот тул сначала
    делает ровно это, а потом проверяет результат живым запросом, так что отвечает
    про состояние на сервере, а не про часы на машине, где крутится MCP.

    Чего не может ни один тул — полного перелогина: он требует пароля и SMS и живёт
    в `.venv/bin/python login_cli.py --myt <логин>`. Если сессия действительно
...
def myt_refresh_session() -> str:
    ...
    (`.venv/bin/python login_cli.py --myt <логин>`), он требует пароля и SMS, и ни
