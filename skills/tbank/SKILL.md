...
| Пополнить телефон; оплата счетов (и почему её нет) | `tbank-bill-pay` |
...
- **Чек по платежу:** `payment_receipt(payment_id)` — PDF. `payment_id` берётся из
  ответа `transfer()` или из `orders()`, потом его взять негде.
...
- **Кредиты, вклады, справки и прочее:** `get_data(section)` — десятки разделов,
  список в описании тула. Две секции (`providers`, `requisites`) — фильтры и требуют
  второй аргумент.
...
1. **Деньги — только после подтверждения конкретной суммы.** `transfer`,
   `grocery_checkout`, `ticket_pay` списывают по-настоящему. Просьба «купи»,
...
2. **Не выдумывай данные.** Нет КБЖУ у товара — скажи «не указано». Не знаешь id
   провайдера — скажи, что оплатить нельзя. Выдуманное число или id хуже отсутствия.
...
5. **Обрезанный ответ помечен.** Если в выводе «показано N из M» или «ОТВЕТ ОБРЕЗАН» —
   не считай по нему итогов, запроси остальное через `limit`.
...
- оплата ЖКХ, налогов, штрафов, интернета — тула нет (комиссию посчитать можно);
...
- отмена билета не подтверждена: в захвате оба пути отвечали 500.
