...
description: |
  ...
  финансы, поезд, самолёт, рейс, авиабилет, маркетплейс — или спрашивает, что тут
  вообще можно. Отсюда уходи в узкий скил, если задача попадает в него; если нет —
...
| Поезда, самолёты, маркетплейс — только поиск, без оформления | `tbank-travel-search` |
...
| Рабочие встречи, расписание, приглашения (MyT) | `myt-calendar` |
| Парковка в офисе, брони рабочих мест (MyT) | `myt-parking` |

## MyT — это не банк

Два последних скила ведут в **рабочее** приложение Т-Банка: корпоративный календарь
и парковка в офисе. У него отдельный логин и отдельная сессия — банковская сюда
доступа не даёт, а `refresh_session()` его не чинит. Проверить: `myt_status()`.
Восстановить: пользователь сам запускает `.venv/bin/python login_cli.py --myt <логин>` в своём
терминале; тула, принимающего корпоративный пароль, здесь нет и не будет.
...
1. **Деньги — только после подтверждения конкретной суммы.** Списывают
   по-настоящему ровно пять тулов: `transfer`, `transfer_requisites`, `pay_bill`,
   `grocery_checkout`, `ticket_pay`. Просьба «купи»,
...
- **покупка ЖД-билета, авиабилета, оформление заказа в маркетплейсе** — только
  поиск, до оплаты ни одна из трёх вертикалей не доходит; подробности и флоу —
  `tbank-travel-search`;
- бронирование концертов со свободной рассадкой — только просмотр: захвата
  заказа именно с этого экрана нет;
