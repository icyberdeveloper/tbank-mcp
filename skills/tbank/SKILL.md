...
- **покупка ЖД-билета** — `train_search()` и `train_calendar()` только ищут:
  оплата уходит в webview, который headless не закрыть. Коды станций числовые,
  по названию банк их не резолвит;
- бронирование концертов со свободной рассадкой — только просмотр: захвата
  заказа именно с этого экрана нет;
- **покупка авиабилета** — `flight_search()` только ищет и сравнивает;
  подтверждённого шага бронирования и оплаты нет. Кодов IATA по названию города
  банк тоже не резолвит — бери их из `flight_history()`;
- **оформление заказа в маркетплейсе** — `shop_search()` и `shop_cart()` только
  показывают товары и корзину; подтверждённого шага размещения заказа нет,
  оплачивать пользователь идёт в приложение;
