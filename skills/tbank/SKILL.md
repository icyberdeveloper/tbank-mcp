...
description: |
  ...
  финансы, поезд, самолёт, рейс, авиабилет, отель, маркетплейс — или спрашивает,
  что тут вообще можно. Отсюда уходи в узкий скил, если задача попадает в него; если нет —
...
| Поезда и самолёты — от поиска до купленного билета и возврата; отели и маркетплейс — поиск | `tbank-travel` |
...
- **Заказы:** `orders(kind)` — продукты, кино, концерты, авиа, поезда, отели в одной
  ...
  `travel_order_details(order_id)` — детали поездки: у ЖД вагон, места и статус
  электронной регистрации, у авиа маршрут и документы, у отеля номер и гости.
  Сами поездки одной лентой — `trips()`; билет в файл —
  `travel_ticket_file(order_id)`.
- **Чек по платежу:** `payment_receipt(payment_id)` — PDF. `payment_id` берётся из
  ...
  взять негде.
- **Сколько поездка стоит на самом деле:** `travel_payment_options(amount)` — чем
  можно заплатить, сколько бонусов спишется, сколько кэшбэка вернётся, какие есть
  рассрочки. Ничего не оплачивает.
...
- **Кредиты, вклады, справки и прочее:** `get_data(section)` — десятки разделов,
  список в описании тула. Семь секций (`sbp_me2me`, `providers`, `requisites`,
  `statements`, `account_details`, `full_debt_amount`, `statement_exist`) — фильтры
  и требуют второй аргумент; без него тул поднимает ошибку, а не отдаёт пустой ответ.
...
1. **Деньги подтверждает кнопка тула, а не твой вопрос.** Списывают по-настоящему
   ровно восемь тулов: `transfer`, `transfer_requisites`, `pay_bill`,
   `grocery_checkout`, `ticket_pay`, `train_pay`, `flight_book` и `confirm_payment`
   (он завершает платёж, который банк держит на втором факторе). Первые семь сами
   показывают пользователю кнопки «Перевести/Оплатить/Оформить … N ₽?» с реальным
   итогом и отправляют платёж только после нажатия. `train_book` (держит места,
   но не списывает) и `train_refund` (возврат — это отмена) кнопки НЕ показывают;
   `train_refund` вместо неё сначала печатает расчёт и требует `confirm=True`. Просьба «купи», «переведи», «закажи» — это **запрос**, а
   не подтверждение, но отвечать на него надо вызовом тула: покажи то, чего на
   кнопке нет (получатель и комиссия, состав корзины, места и сбор) — и вызывай.
   Своё «да/нет» текстом не спрашивай: в клиенте с кнопками это второй вопрос об
   одном и том же. Клиент без элиситации получит «ПЛАТЁЖ НЕ ВЫПОЛНЕН» — деньги
   там не двигаются вообще; скажи это прямо, обходных путей не ищи.
...
- **бронирование отеля и оформление заказа в маркетплейсе** — только поиск;
  у отеля тариф даже несёт код брони, но вызова, который его примет, в API нет.
  Поезда и самолёты, наоборот, покупаются целиком — флоу в `tbank-travel`;
- **возврат авиабилета** — в API банка его нет: «гарантированный возврат» это
  допуслуга, которую покупают вместе с билетом. ЖД-билет вернуть можно —
  `train_refund`;
